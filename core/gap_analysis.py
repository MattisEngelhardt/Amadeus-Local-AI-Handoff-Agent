...
MATERIAL_HINT_PATTERNS = [
    ...
    r"\bquelle(n)?\b",
    r"\battachment(s)?\b",
    r"\battached\s+file(s)?\b",
    r"\bprovided\s+file(s)?\b",
    r"\bsource\s+file(s)?\b",
    r"\buploaded\s+file(s)?\b",
]
            ...
            for m in failed_materials:
                result.blockers.append(
                    self._gap(
                        ...
                        (
                            f"Can you provide '{m.original_path}' in a supported "
                            "format (like .md or .txt)?"
                        ),
                    )
                )
